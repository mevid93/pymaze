...
Tama moduuli sisaltaa Weave labyrinttien luomiseksi suunnitellun algoritmin.
'''
...
def luoWeaveLabyrintti(leveys, korkeus):
    pixelX = 275
    pixelY = 25
    pixelSivu = 25
    ...
    for y in range(korkeus):
        for x in range(leveys):
            ...
            elif(sisaltaaSuunnat(['SY', 'W', 'E', 'NY'], lista[y][x])):
                lista[y][x] = YlikulkuPystysuuntaPala(pixelX, pixelY, pixelSivu, pixelSivu)
            elif(sisaltaaSuunnat(['S', 'WY', 'EY', 'N'], lista[y][x])):
                lista[y][x] = YlikulkuVaakasuuntaPala(pixelX, pixelY, pixelSivu, pixelSivu)
            else:
...
def recursiveBactracking(leveys, korkeus, x, y, maaliX, maaliY, lista, suunta):
    ...
    for i in range(0,4):
        ...
        if(seuraavaX < leveys and seuraavaX >= 0 and seuraavaY >= 0 and seuraavaY < korkeus):
            ...
            elif(ylitysX < leveys and ylitysX >= 0 and ylitysY >= 0 and ylitysY < korkeus):
                if not lista[int(ylitysY)][int(ylitysX)]:
                    if(sisaltaaSuunnat(lista[int(seuraavaY)][int(seuraavaX)], ['W', 'E']) and (suunta == 'N' or suunta == 'S')):
                        lista[int(y)][int(x)].append(suunta)
                        lista[int(seuraavaY)][int(seuraavaX)] = ['NY', 'SY', 'E', 'W']
                        lista = recursiveBactracking(leveys, korkeus, ylitysX, ylitysY, maaliX, maaliY, lista, suunta)
                    elif(sisaltaaSuunnat(lista[int(seuraavaY)][int(seuraavaX)], ['N', 'S']) and (suunta == 'W' or suunta == 'E')):
                        lista[int(y)][int(x)].append(suunta)
                        lista[int(seuraavaY)][int(seuraavaX)] = ['N', 'S', 'EY', 'WY']
                        lista = recursiveBactracking(leveys, korkeus, ylitysX, ylitysY, maaliX, maaliY, lista, suunta)
    return lista
